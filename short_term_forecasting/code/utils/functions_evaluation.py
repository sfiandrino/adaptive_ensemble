...
def import_baseline(date, url):
    ...
    url_all = url + '/' + filename
    df = read_csv_from_github(url_all)
    return df

def import_ens_flusight(date, url):
...
def compute_dict_WIS_AE_forecast(df):
    dict_wis_horizon = {}
    dict_ae_horizon = {}
    list_horizons = df['horizon'].unique().tolist()
    if -1 in list_horizons:
        list_horizons.remove(-1)
