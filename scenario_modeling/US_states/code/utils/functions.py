...
def loading_surveillance(ref_date, start_date, github_repo, github_directory, state_code, season):
    """
    ...
    """
    print("Loading surveillance data...", ref_date)
    if ref_date == datetime(2024, 11, 23).date():
        ref_date_next = datetime(2024, 11, 30).date()
        file = f"target-hospital-admissions_{ref_date_next}.csv"
        file_url = f"https://raw.githubusercontent.com/{github_repo}/main/{github_directory}/{file}"
        df_surv = read_csv_from_github(file_url)
    elif ref_date == datetime(2024, 12, 7).date():
        ref_date_next = datetime(2024, 12, 14).date()  # Adjust for the specific case in the 2024-2025 season
        file = f"target-hospital-admissions_{ref_date_next}.csv"
        file_url = f"https://raw.githubusercontent.com/{github_repo}/main/{github_directory}/{file}"
        df_surv = read_csv_from_github(file_url)
    elif ref_date == datetime(2025, 1, 4).date():
        ref_date_next = datetime(2025, 1, 11).date()
        file = f"target-hospital-admissions_{ref_date_next}.csv"
        file_url = f"https://raw.githubusercontent.com/{github_repo}/main/{github_directory}/{file}"
        df_surv = read_csv_from_github(file_url)
    elif ref_date == datetime(2025, 1, 18).date():
        ref_date_next = datetime(2025, 1, 25).date()
        file = f"target-hospital-admissions_{ref_date_next}.csv"
        file_url = f"https://raw.githubusercontent.com/{github_repo}/main/{github_directory}/{file}"
        df_surv = read_csv_from_github(file_url)
    else:
    ...
    df_surv_date_state = df_surv[(df_surv.location == state_code) & 
...
def ranking_trajs(dict_score_traj, toptraj_score, df, season):
    """
    ...
    for scenario in scenarios:
        if season == "2024-2025":
            count = sum(scenario in traj for traj in all_keeptrajs) # count the number of trajectories for each scenario
        else:
...
def computing_wmape_trajs(df, df_surv, list_hor, season):
    """
    ...
    for col in df.columns:
        ...
        if season == "2024-2025":
            modelname = col.split('2024-08-01')[0][:-3]
            print(modelname)
        else:
            modelname = col.split('_')[2]
