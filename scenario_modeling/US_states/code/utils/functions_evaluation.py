...
def loading_surveillance_eval(start_date, github_repo, github_directory, surveillance_file, horizon_to_start):
    """
    ...
    output:
        df_surv_date = dataframe with the filtered surveillance data
    """
    ...
    df_surv_date = df_surv[(df_surv.date >= start_date)]
    df_surv_date = df_surv_date.rename(columns={"value": "hospitalizations"})
    df_surv_date = df_surv_date.sort_values(by='date')
    df_surv_date['horizon'] = df_surv_date.groupby('location').cumcount() + 1
    if 'X' in df_surv_date.columns:
        df_surv_date.drop(columns=['X'], inplace=True)
    df_surv_date = df_surv_date[df_surv_date['horizon'] >= horizon_to_start]
    df_surv_date['date'] = pd.to_datetime(df_surv_date['date'])
    return df_surv_date
