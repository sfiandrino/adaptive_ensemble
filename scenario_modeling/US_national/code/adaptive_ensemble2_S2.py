...
if __name__ == "__main__":
    ...
    print(df_scenarios.model_name.unique())
    ...
    if season == '2023-2024':
        ...
        end_date = datetime(2025, 5, 10) # the last round of the ensemble ends on this date
    ...
    for h in df_scenarios['horizon'].unique().astype(int)[round_init:end_round]:
        ...
        if ref_date == (datetime(2025, 1, 25)):
            continue #this is the date in which CDC data were not available and we have to skip this round coherently 
        ...
        for k in k_values:
            ...
            ens_r = r.r['ensemble_lop'](dfQ_k_r_r, h, k, day_tosave, path_to_save, loss_function, is_original, scenario, season)
    ...
    if season == '2023-2024':
        ...
        df_jaccard_to.index = pd.date_range(start='2024-11-30', periods=len(df_jaccard_to), freq='W-SAT')
        df_jaccard_prev_h.index = pd.date_range(start='2024-12-07', periods=len(df_jaccard_prev_h), freq='W-SAT')
    df_jaccard_to.to_csv(path_persistence + f"Jaccard_index_t0_Ens2_S2_LOP_{loss_function}_{season}.csv")
    df_jaccard_prev_h.to_csv(path_persistence + f"Jaccard_index_tprevious_Ens2_S2_LOP_{loss_function}_{season}.csv")

    # POSTERIOR DISTRIBUTION ANALYSIS
    if season == '2023-2024':
        start_week = datetime.strptime("2023-11-04", "%Y-%m-%d")
    elif season == '2024-2025':
        start_week = datetime.strptime("2024-11-30", "%Y-%m-%d")
    posterior_rows = []
    ...
    df_posteriors.to_csv(path_posterior + f"posterior_distribution_Ens2_S2_LOP_{loss_function}_{season}.csv", index=False)