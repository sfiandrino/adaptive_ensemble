...
ensemble_lop <- function(df, hor, toptraj, day_tosave, path_tosave, loss_function, is_original, scenari, n_bootstrap) {
  ...
  weights <- data.frame(id = c('MOBS_NEU-GLEAM_FLU', 'NIH-Flu_TS', 'NotreDame-FRED',
                  'PSI-M2', 'USC-SIkJalpha', 'UT-ImmunoSEIRS', 'UVA-FluXSim', 'UVA-EscapeFlu'), weight = c(1, 1, 1, 1, 1, 1 ,0.5, 0.5))
  ...
  if (is_original == TRUE) {
    ...
    finalpath <- paste0(path_tosave, day_tosave, "_", toptraj, "_", loss_function, "_", n_bootstrap, ".csv")
  }
