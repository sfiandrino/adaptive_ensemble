...
ensemble_lop <- function(df, hor, toptraj, day_tosave, loss_function, is_original, scenario, season, USstate) {
  ...
  if (season == "2024-2025") {
    ...
    weights <- data.frame(id = c('UIUC-IMMCYC', 'JHU_UNC-flepiMoP', 'UVA-adaptive', 'UNCC-Hierbin',
 'MOBS_NEU-GLEAM_COVID', 'PSI-M_CoV_2025', 'CFA-Scenarios', 'UT-ImmunoSEIRS'), weight = c(1, 1, 1, 1, 1, 1, 1, 1))
  }
  ...
  #if (is_original == TRUE) {
  #  finalpath <- paste0(path_tosave, "Ensemble_", scenario, "_", USstate, "_", season, ".csv")
  #} else {
  #  finalpath <- paste0(path_tosave, day_tosave, "_", toptraj, "_", USstate, "_", loss_function, "_", season, ".csv")
  #}
