...
def plot_scenario_polar(df, k_value, output_path, age_strata, font_prop=None, font_prop_title=None, season=None, skipping_round = False):
    ...
    df_k_pivot['week'] = pd.to_datetime(df_k_pivot['week'])
    iso = df_k_pivot['week'].dt.isocalendar()
    df_k_pivot['date_week'] = iso['year'].astype(str).str[-2:] + '-' + iso['week'].astype(str).str.zfill(2)
    ...
    for scenario, ax in zip(scenarios, axes.ravel()):
        ...
        for i, (angle, length, max_value, week_date) in enumerate(zip(angles, lengths, max_per_week, df_k_pivot["week"])):
            if skipping_round and pd.to_datetime(week_date).strftime('%Y-%m-%d') == skipping_round:
                continue  # salta lo spicchio, ma manterremo l'etichetta
    ...
    plt.tight_layout()
    if age_strata == True:
        filename = f"{output_path}/scenario_posterior_k{k_value}_main_{season}_agestrata.png"
    else:
        filename = f"{output_path}/scenario_posterior_k{k_value}_main_{season}.png"
